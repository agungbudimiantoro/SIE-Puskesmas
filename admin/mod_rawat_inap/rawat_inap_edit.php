...
<div class="row">
    <div class="col-md-6">
        <form action="?p=rawat_inap_proses" method="POST">
            ...
            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label">nama kamar</label>
                <select class="form-select" aria-label="Default select example" name="kode_kamar" required>
                    <option disabled selected>nama kamar</option>
                    <?php
                    $select_query = mysqli_query($conn, "SELECT * FROM kamar");
                    while ($data_sel = mysqli_fetch_assoc($select_query)) {
                        if ($data['kode_kamar'] == $data_sel['kode_kamar']) {
                    ?>
                            <option selected value="<?= $data_sel['kode_kamar'] ?>"><?= $data_sel['nama_kamar'] ?></option>
                        <?php } else { ?>
                            <option value="<?= $data_sel['kode_kamar'] ?>"><?= $data_sel['nama_kamar'] ?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
            </div>
